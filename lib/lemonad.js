...
(function() {
  ...
  L.nth = function(a,index) {
    ...
    if (!_.isArray(a)) fail("Indexing not supported on non-array type.");
  ...
  L.meth = L.walterWhite = L.invoker = function(method) {
    return function(target /* args ... */) {
      ...
      if (L.existy(targetMethod) && (method === targetMethod)) {
        return targetMethod.apply(target, _.rest(arguments));
      }
      else {
        return undefined;
      }
  ...
  // if it "looks good".  Validation functions also
  // have a `message` field useful for building human-readbale
  // error messages.
  L.validator = function(message, fun) {
    var f = function(/* args */) {
