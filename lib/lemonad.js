...
(function() {
  ...
  L.gt  = function(lhs, rhs) { return lhs > rhs; };
  L.lt  = function(lhs, rhs) { return lhs < rhs; };
  ...
  L.nth = function(a,index) {
    ...
    if (!L.isIndexed(a)) fail("Indexing not supported on non-indexed types.");
    if ((index < 0) || (index > a.length - 1)) fail("Attempting to index outside the boubds of the array.");
  ...
  L.isReference = function(x) { return x.constructor === L.Hole; };
  ...
  L.meth = L.walterWhite = L.invoker = function(method /*, args*/) {
    ...
    return function(target /* args ... */) {
      ...
      if (L.existy(targetMethod) && (method === targetMethod)) {
        return targetMethod.apply(target, L.cat(pargs, _.rest(arguments)));
      }
      else {
        return undefined;
      }
  ...
  L.curry = L.curry1 = L.fix1 = function(fun) {
    return function(arg) {
      return fun(arg);
    };
  };
  ...
  L.$.index = function(xrel, ks) {
    return _.reduce(xrel, function(index, rel) {
      var ik = _.pick.apply(null, L.cons(rel, ks));
  ...
  L.$.join = function(xrel, yrel) {
    var xks = L.$.set(keys(_.first(xrel)));
    var yks = L.$.set(keys(_.first(yrel)));
    ...
    return _.reduce(s, function(result, o){
      var found = L.$.lookup(idx, _.pluck.apply(null, L.cons(o, ks)));
