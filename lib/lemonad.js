...
(function() {
  ...
  var currier = function(direction) {
    // a curry function constructor allowing for either direction,
    // that is, right-to-left or left-to-right.
  	var binder = direction == 1 ? bindLeft : bindRight;
  	return function(fun, count) { 
  	  if( count === undefined ) {
  	    // if no count was provided, parse the function or default to zero 
  ...
  // currying and partial application built up from the currier.
  L.curry = currier(-1);  
