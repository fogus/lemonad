...
(function() {
  ...
  var _ = this._ || require('underscore-contrib');
  ...
  L.bindRight = function(fun, arg) {
  	return function(/* args... */) {
  		return fun.apply({}, [].slice.call(arguments).concat(arg));
  	};
  };
  L.bindLeft = function(fun, arg) {
  	return fun.bind({}, arg);
  };
  L.currier = function(direction) {
  	var binder = direction == 1 ? L.bindLeft : L.bindRight;
  	return function(fun, count) {  	
  	  ...
  	  if( !count ) {
  	    if( fun.toString().match(/\(([^)]+)\)/) ) {
  	      count = fun.toString().match(/\(([^)]+)\)/)[1].split(/,\s?/).length;
  	    } else {
  	      count = 0;
  	    }
  	  }  
  ...
  L.curry = L.currier(-1);
  L.partialler = function(fun, count) {
    // a constructor of a function which will perform
    // ... partial application on a given function.
    return function(/* args... */) {
  	  var args = [].slice.call(arguments);
  	  // call with args one by one, and return a function
  	  // ... prepared to do the same.
  	  return L.uncurry(L.uncurry(L.currier(1)(fun)).apply({}, args));
    };	  
  };
  L.partial = function(fun/*, args... */) {
    // partial applier of a given function
    return L.partialler(fun).apply({}, [].slice.call(arguments, 1));
  };
