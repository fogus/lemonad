...
(function() {
  ...
  L.butLast = function(coll) {
    if (!L.isSeq(coll)) throw "Cannot butLast a non-sequence";
  ...
  L.splitAt = function(index, coll) {
    if (!_.isNumber(index)) throw "expected a number for the first arg";
    if (!L.isSeq(coll)) throw "expected an array for the second arg";
  ...
  L.takeSkipping = function(n, coll) {
    if (!_.isNumber(n)) throw "expected a number for the first arg";
    if (!_.isArray(coll)) throw "expected an array for the second arg";
